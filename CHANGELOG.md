...
- **Four shell-parser segmentation misses that hid a command from every block-capable guard (cameronsjo/cadence-hooks#490, #491, #496, #497).** All four are the same class — the dangerous command escapes inspection entirely rather than being wrongly blocked — and all four were measured against a built binary, each payload paired with a known-bad twin returning **Block** in the same run. Across the two probe scripts, 17 verdicts flipped Allow → Block and none flipped the other way.
  ...
  **Three residual misses are documented rather than fixed, all pre-existing and all outside this parser.** `prevent-secret-leaks` lowercases the whole command upstream, so sudo's case-sensitive short flags arrive folded (`-E` as `-e`) and miss the new allowlist — that guard alone still allows `sudo -E bash -c 'cat .env'`. `guard-gh-dangerous` pass 1 matches against `strip_quotes(command)` rather than segments, so an unmatched apostrophe anywhere suppresses it, with or without a comment. And `guard-rm` strips a glued trailing `)` or `}` from an operand but not a backtick, so `rm -rf ~/Documents\`` is allowed on its own, independent of any segmentation.
