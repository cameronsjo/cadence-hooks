...
- **guard-rm stopped approving three shapes of protected delete** (cameronsjo/cadence-hooks#428, #426, #427). All three were silent ALLOWs on `main` — not prompts, not blocks — and all three were filed together as sharing a primitive with `enforce_worktree`. Each is verified by a 22-case probe against a real binary with every kill switch explicitly unset, showing the shapes ALLOW before and BLOCK-or-ASK after, alongside controls that must stay silent.
  ...
  **Every spelling of a directory change is now followed (#428).** The check compared the raw first token against the literal `cd`, so `command cd`, `builtin cd`, `\cd`, `FOO=1 cd` and `pushd` all left the effective directory stale and a later relative target was judged against a directory the shell had already left — `command cd ~; rm -rf Documents` from a temp cwd read as a temp delete and was approved. It now routes through `command_word`, the same basename and `\`-strip the delete-verb check uses, after stepping over assignment words and the shell-preserving prefixes.
  ...
  `env`, `nice` and `nohup` are deliberately **not** stepped over: they are programs that run their argument as a child process, where a `cd` cannot move the parent's `$PWD` at all, so treating them as directory changes would invent a move the shell never made. `time` and `eval` are the counter-examples and are included — `time` is a shell *reserved word*, not a program, so `time cd ~` runs in the current shell and does move `$PWD` (verified in bash and zsh); `eval` likewise. The prefix match runs through `command_word`, so the backslash forms (`\command cd`, `\builtin cd`) are caught too — the same escape the fix closes for `\cd`, one token to the left.

  `pushd`/`popd` are modeled with a real directory stack rather than collapsed to "directory unknown". Collapsing is safe but noisy — a bare `popd` with no prior `pushd` leaves the shell exactly where it was, so declaring defeat would turn `popd; rm notes.txt` into a prompt about a file in a directory the parser can name perfectly well.

  **Their options are read, not skipped, because they change what the builtin does.** `pushd -n <dir>` pushes and *stays*; treating `-n` as skippable noise consumed `<dir>` as a `cd` target and moved the tracked directory where the shell had not — from `$HOME`, `pushd -n /tmp; rm -rf Documents` then read as a temp delete while bash removed `~/Documents`. That was a BLOCK turned into a silent ALLOW by the stack model itself, caught by adversarial review before it shipped. `popd -n` and `popd +N` likewise drop a stack entry without moving, and `+N`/`-N` rotations move to an entry this parser never named, so they mark the directory unknown rather than guessing.
