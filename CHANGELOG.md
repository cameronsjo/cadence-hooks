...
- **`parse_work_dir` now reads shell segments instead of scanning the raw command string, fixing a false-nudge and a quote-hijack in the same regex (cameronsjo/cadence-hooks#394, #368).** Both issues were filed as a writer/reader keying divergence in the polish marker — worktree gitdir vs common dir. That diagnosis is **wrong**: `record-polish` and `polish_marker_present` have both keyed on the canonicalized `git_common_dir` since 0.60.0, and the clean worktree case reproduces silent. The real defect was in the *reader's cwd resolver*. `CD_PATTERN` matched a `cd` anywhere in the raw string against a separator alternation of `&&`/`;`/`||`, and a newline was **neither a separator nor excluded from the bare-path character class** — so the real failing shape, `cd <worktree>` ⏎ `gh pr create`, captured the target as `<worktree>\ngh`. That directory does not exist, `GitState::resolve` returned `None`, and the gate nudged. The marker had been correct the whole time. Newline-separated, subshell-wrapped (`( cd /wt && … )`), and glued-group forms all resolve now.

  **The same regex had an inverse bug, and that half is security-sensitive:** it was not quote-aware, so a `cd` inside a quoted *argument* — `gh pr create --body "step one; cd /some/worktree"` — hijacked resolution and silently redirected the target. `parse_work_dir` feeds three **block-capable** guards (`git-safety`, `guard-gh-write`, `guard-push-remote`), so that direction let a body string move a guard's idea of which repository it was protecting. Resolution now walks `split_segments`, which strips heredoc bodies, is quote-aware, and treats `'\n'` as a first-class operator — the three properties the raw scan lacked. A `cd` applies iff it is a segment's command word; one inside a quoted argument or a heredoc body never applies. Left-to-right accumulation and the assume-success model (#229) are unchanged.

  Segmenting also surfaced a third case the raw scan had been hiding by accident: a `cd` in a **pipeline** or a **backgrounded** one (`echo x | cd /sub`, `cd /sub & gh pr create`) runs in a subshell and leaves the parent's directory alone. The old alternation held only `&&`/`;`/`||`, so a single `|`/`&` neighbour suppressed the match for the wrong reason — and a segment walk that ignored the operators would have started *applying* those, steering the same three block-capable guards onto a directory the command never enters. The walk now reads `split_segments_with_ops` and skips a segment whose preceding operator is `|` or whose following operator is `|`/`&`. A `&` backgrounding some *other* command still leaves a following `cd` in the parent, so that one applies.
