...
- **`guard-rm` no longer blocks a symlink for the repo it merely points at (cameronsjo/cadence-hooks#402).** `rm`/`unlink` on a symlink removes the LINK — the repo on the other side is unreachable by that operation. The git-root probe stats `<target>/.git`, and that `stat` **follows the link**, so a symlink to a checkout reported `GitRepo` and blocked on a repo the command could never touch. A new injected `is_symlink` probe settles it with an `lstat` (`symlink_metadata`, which does not follow the final component — `metadata` would agree with the git-root probe and answer the wrong question), and a `GitRepo` operand that is itself a link demotes to the scratch class.
  ...
  **The demotion applies only when the operand names the link ITSELF, and three guards enforce that** — each closing a route where the shell really does resolve through the link:
  ...
  The first two are carried as a `dereferences` flag computed in `resolve_target`, because by the time the judge sees a resolved path the spellings are indistinguishable — all of them reduce to `link`. The guards are ordered cheapest-first, so two field tests and a string scan run before the `lstat`, which is the only syscall in the chain. The demotion also stays `GitRepo`-only: a symlink that is a home child or sits inside the vault is classified by those rules first and never reaches the arm.

  Known and unchanged: a destructive `find` reaches this arm through its search roots, and `find -L <link> -delete` genuinely does dereference — so that one spelling now allows where it previously blocked. Left as a documented miss (it is enumerated in the module header) rather than closed, consistent with the guard's charter — a discipline guard, not a security boundary, whose threat model is accidents.
