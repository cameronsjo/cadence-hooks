...
- **guard-rm stopped approving three shapes of protected delete** (cameronsjo/cadence-hooks#428, #426, #427). All three were silent ALLOWs on `main` — not prompts, not blocks — and all three were filed together as sharing a primitive with `enforce_worktree`. Each is verified by a 22-case probe against a real binary with every kill switch explicitly unset, showing the shapes ALLOW before and BLOCK-or-ASK after, alongside controls that must stay silent.
  ...
  **A real `pushd`/`popd` stack was built here and withdrawn.** It existed only to spare a prompt on `popd; rm notes.txt`, and it produced a BLOCK-to-silent-ALLOW regression in two consecutive adversarial review rounds: first `pushd -n` consuming its operand as a `cd` target (from `$HOME`, `pushd -n /tmp; rm -rf Documents` read as a temp delete while bash removed `~/Documents`), then the fix for that inverting the model for a trailing `-n`. The plain-spelling fixes beside it stayed clean throughout all three rounds.

  This is the same call the same-command variable expansion got, for the same reason: when each round of enumeration finds the next *route* rather than a variation of the last, the feature is the defect. Nothing regresses by removing it — the affected shapes return to prompting, which is what they did before. The cost is one extra prompt on `popd; rm notes.txt`, paid deliberately.
