...
- **guard-rm stopped approving three shapes of protected delete** (cameronsjo/cadence-hooks#428, #426, #427). All three were silent ALLOWs on `main` — not prompts, not blocks — and all three were filed together as sharing a primitive with `enforce_worktree`. Each is verified by a 22-case probe against a real binary with every kill switch explicitly unset, showing the shapes ALLOW before and BLOCK-or-ASK after, alongside controls that must stay silent.

  **No spelling of a directory change is a silent ALLOW any more (#428).** The check compared the raw first token against the literal `cd`, so `command cd`, `builtin cd`, `\cd`, `FOO=1 cd` and `pushd` all left the effective directory stale and a later relative target was judged against a directory the shell had already left — `command cd ~; rm -rf Documents` from a temp cwd read as a temp delete and was approved.

  **What is modeled is one thing: a bare `cd` after assignment words.** That is the only directory change every shell agrees about. Verbs are matched with **at most one** leading backslash removed and never a basename, so `\cd` resolves while `\\cd`, `/usr/bin/cd` and `./cd` do not — those are programs (macOS really ships `/usr/bin/cd` as a `#!/bin/sh` wrapper running `builtin cd` in a child), and no shell's parent moves for them, so the directory is correctly retained.

  Everything else — `pushd`, `popd`, and any verb reached through `command`/`builtin`/`eval`/`time` — marks the position **unknown**, which asks. `dir_known` is never restored to true, so a later `cd` cannot launder an unknown position back into a confident verdict.

  **Three features were built here and withdrawn**, each after adversarial review found it turning a BLOCK into a silent ALLOW: a `pushd`/`popd` stack, then a prefix layer that judged from a word's *name* whether it keeps the following builtin in this shell, then `pushd` operand resolution. Four review passes, four rounds of defects, two mechanisms:

  | | zsh 5.9 | bash 5.3 | bash 3.2 |
  |---|---|---|---|
  | `command cd <dir>` | **stays** | moves | moves |
  | `pushd <dir> -n` | **stays** | **stays** | moves |
  | `pushd <old> <new>` | **name substitution** | **error** | moves |

  `command` forces an *external* lookup in zsh; `pushd`'s option grammar and its operand grammar each diverge across shells; and `strip_group_wrappers` erases the subshell, background and pipeline contexts where a `pushd` cannot move the parent at all. Every round answered for one shell and was wrong in another — including a round whose ground truth came from `/bin/bash`, which on macOS is 3.2, the single shell where the trailing-`-n` case moves.

  **The result is a narrower fix than the issue proposed, stated plainly.** Every row of #428's table moves off silent ALLOW: `\cd` and `FOO=1 cd` reach BLOCK, while `command cd`, `builtin cd` and `pushd` reach ASK. A prefixed form that `main` blocked — because main tracked no directory change at all, so it retained the real directory — now asks as well. That is a weakening on those shapes and it is deliberate: retaining is only safe when *no* shell moves, and for these the shells disagree.

  **A transparent prefix carrying a flag no longer reads as "not a deletion" (#426).** `skip_transparent_prefixes` only skips a prefix when the *next* token is not an option, so `env -i rm -rf ~/Documents` left `env` as the leading word, matched no delete verb, collected no targets — and an empty target list meant "not a deletion". The segment is now **unresolvable** instead, which asks rather than approves. `eval` joins the same gate — `eval rm -rf ~/Documents` is an explicit row in that issue's table and was in neither prefix set. Fixed guard-rm-locally rather than in the shared primitive: that one is also `enforce_worktree`'s and carries its own review history, where the same clause is a documented miss whose cost is only a missed commit block. Unresolvable rather than a re-parse because each prefix has its own flag grammar — `env -u` and `nice -n` take a value, `env -i` does not — and guessing wrong would skip past the delete verb itself. Gated on a delete verb actually appearing, so `env -i ls` stays silent.

  The gate keys on a literal delete-verb *token*, which a deletion does not have to carry: `env -i find ~/Documents -delete` and `env -i sh -c "rm -rf ~/Documents"` both still allow. Those are pre-existing — the unprefixed forms block, and the prefix alone defeats them — and are filed rather than widened into this change.

  **A `..` behind a glob metachar no longer escapes the parent-segment check (#427).** `glob_literal_prefix` truncates the operand at the first `*`/`?`/`[`, and the check read only that truncated prefix — so `rm -rf /private/tmp/*/../../..`, which every match expands to `/`, classified as a clean glob under `/private/tmp` and was approved. The check now runs against the whole operand before any reduction. Same reasoning as the brace guard three lines above, which sat directly on top of the truncation that defeated it for globs.

  **Not fixed here, filed as #441:** `enforce_worktree` carries the identical literal-`cd` comparison at two sites, and the same five spellings evade its commit block. Deferred deliberately — PR #415 is parked open on that file with 23 Windows fail-opens, and the fix wants its own review pass and its own Windows verification rather than riding this one.
