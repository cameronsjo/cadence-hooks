...
- **`tests/codex_coverage.rs` no longer writes telemetry into the operator's real metrics directory.** Both spawn sites ran the built binary without pinning `CADENCE_METRICS_DIR`, so `metrics_dir()` resolved to the live ledger and running the suite appended production-shaped rows to it — and, once the default moves off `~/.claude/metrics`, created that directory on the developer's machine as a side effect of `cargo test`. Every other integration test in the directory already pins it; these two now do too, via an `isolated_cadence_hooks` helper.
