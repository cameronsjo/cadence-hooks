...
- **`guard-gh-write` stops blocking two commands that were correctly spelled** (cameronsjo/cadence-hooks#454, #457). Both are false BLOCKs on legitimate reads and writes, and each was reported alongside a workaround the operator had to find by trial.
  ...
  **One gap stays open, and it is now asymmetric enough to name.** Only the FIRST positional is read, so a target placed after a flag is still missed — `gh repo edit --description x evil/repo` is a valid gh spelling (cobra accepts interspersed arguments) that resolves from the cwd remote and is therefore ALLOWED from an owned checkout. That is pre-existing and shared by every verb in the set, not introduced here; what is new is that the same write now blocks when the positional comes first, so the two spellings disagree. Closing it needs gh's per-subcommand flag table to tell a flag's value from the positional — the table `repo_flag` already refuses to grow, and for the same reason: skipping the token after every flag would swallow a real positional following a boolean like `--accept-visibility-change-consequences`, while picking the first `owner/repo`-shaped token would false-BLOCK an ordinary `gh repo edit --description "some/thing"`. Left as a follow-up rather than guessed at, and pinned by a characterization test that must flip to a BLOCK when it is fixed.

  Verified by differential: the new cases spliced onto the pre-change source fail on exactly the ten that probe the two defects, while every negative control passes on both sides — fields with no explicit method, `-X POST`, four disagreeing-method forms, a quoted `-X GET` decoy, a non-`gh api` write, the adjacent `gh api -X POST search/issues` block, `gh repo edit` with no positional falling back to the cwd, and the `gh repo edit` verb appearing only in quoted prose.
