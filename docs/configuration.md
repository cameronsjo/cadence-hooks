...
A name in `CADENCE_DISABLE` is not automatically honoured. Every name resolves to exactly one of three verdicts, and `cadence-hooks list`, `cadence-hooks doctor` and `cadence-hooks configure --list` all report it in the same words:

- **Disabled via CADENCE_DISABLE** — honoured. The hook does not run.
- **Protected — disable refused, these still run** — the name is a guard against irreversible harm (secret exposure, data loss, destructive git/gh/remote/vault operations). `CADENCE_DISABLE` cannot switch these off, because it is silent and persistent and can be set in a repository's committed `settings.json`. The hook runs anyway. Use `CADENCE_BYPASS=1` for a one-session maintenance escape.
- **Named in CADENCE_DISABLE but not a hook, so nothing was disabled** — the name matches no registered hook. Matching is exact and case-sensitive, so `Guard-Rm`, `guard_rm` and `guard-rm-liveness` are near-misses, not fuzzy matches, and they disable nothing. Run `cadence-hooks list` for the canonical names.
