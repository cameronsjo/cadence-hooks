...
| `prevent-secret-leaks` | PreToolUse (Read, Grep, Bash) | Block reading .env, credentials, private keys (exempt: a **bare** `forgectl env keys\|set\|get\|check\|redact` naming its `--file` target — not a path-qualified or wrapped spelling, not another operand, not a redirection) |
